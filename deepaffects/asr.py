...
class AsrApi():
    ...
    def async_asr(
        ...
        request_json['enablePunctuation'] = enablePunctuation
        request_json['enableSpeakerDiarization'] = enableSpeakerDiarization

        if source:
            request_json['source'] = source

        output = requests.post(url=AsrApi.URL, json=request_json, params=params).json()
